...
- Био-индекс RAMZES: ${bioIndex}/10`;
  ...
  const sendWithMessages = async (msgs: Message[]) => {
    ...
    try {
      ...
      if (data?.error) {
        const errMsg = ⚠️ Ошибка API (${data.error.code}): ${data.error.message?.slice(0, 50)};
  ...
  return (
    <div className="glass-panel rounded-lg p-4 glow-border transition-all duration-300 h-full flex flex-col bg-background/50">
      <div className="flex items-center gap-2 mb-3">
        ...
        <div className="ml-auto flex gap-1">
          ...
          <button onClick={() => setMessages([])} className="text-muted-foreground hover:text-primary p-1">
      ...
      {showSettings && (
        <div className="bg-muted/50 rounded-md p-3 mb-3 border border-primary/10">
        ...
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] rounded-lg px-3 py-2 text-sm ${
              m.role === 'user' ? 'bg-primary/20 border border-primary/30' : 'bg-muted/50 border border-border'
            }`}>
              {m.content}
            </div>
          </div>
        ))}
        {loading && <div className="text-xs text-primary animate-pulse">Асириус анализирует...</div>}
      </div>
      ...
      <div className="flex gap-2 mt-auto">
        <input value={input} onChange={e => setInput(e.target.value)}
          ...
          className="bg-input border border-border rounded px-3 py-2 text-sm flex-1 outline-none focus:border-primary/50" />
        <button onClick={sendMessage} disabled={loading} className="bg-primary text-primary-foreground p-2 rounded disabled:opacity-50">
